...
Add a link parameter `-pg` during compilation; if using `make`, it would be `CFLAGS+= -lg`.
...
* time: The percentage of total execution time spent in the function.
* cumulative seconds: The sum of the execution time of the current function and all functions above it.
* self seconds: The execution time of the current function alone.
* calls: The total number of times the function was called. If the function was never called, or the number of calls could not be determined (possibly because the function was not compiled with profiling enabled), the calls field is empty.
* self ms/call: If this function was profiled, the average number of microseconds per call; otherwise empty.
* total ms/call: If this function was profiled, the average number of microseconds per call including its child functions; otherwise empty.
